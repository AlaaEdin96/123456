...
@endif
    ...
    <h1 class="page-title">
    <i class="voyager-watch"></i>Today's Appointments 
    </h1>
    ...
    @endif  
     ...
     @endphp
    @if ($moued->is_c == 0)
    <a   title="Edit" data-toggle="modal" data-target="#smallModal{{$moued->id}}" class="btn btn-sm btn-primary pull-right edit">
        <i class="voyager-edit"></i> <span class="hidden-xs hidden-sm">Confirm</span>
        </a>   
    @endif
    ...
    <!--start model--->
    <div class="modal fade" id="smallModal{{$moued->id}}" tabindex="-1" role="dialog" aria-labelledby="smallModal{{$moued->id}}" aria-hidden="true">
        <div class="modal-dialog modal-sm">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
              <h4 class="modal-title" id="myModalLabel">Appoetment Confirmation</h4>
            </div>
            <div class="modal-body" style="text-align: left">
               <h5>File Number:{{$moued->id_file}}</h5> 
               <h5>Full Name:{{$moued->name}}</h5>
              
               <h5>Phone Number : {{$moued->pohne_number}}</h5> 
               <h5>Sex : {{$moued->sex}} </h5> 
               <h5>Date of Birth : {{$moued->born}} </h5> 
              <h5>Appoetment Date : {{$moued->date}}</h5> 
               <h5>Nationality : {{$moued->madenh->name}}</h5> 
               
               <h5>Appoetment with: {{$moued->appoetment_with}}</h5> 

               <h5>Orginization : {{$moued->monadamh->name}}</h5> 

               <h3>Code : {{$moued->monadamh->code}}</h3> 
                           
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
              <button type="button" class="btn btn-primary" wire:click.prevent="login({{$moued->id}})">Confirm</button>
            </div>
          </div>
        </div>
      </div>      
